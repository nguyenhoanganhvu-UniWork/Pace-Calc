...
function handleReset() {
  ...
  //active list takes what current unit list is then only reset the data of that current unit list
  const activeList= document.querySelector(`.results-list[data-unit-list="${currentUnit}"]`);
    activeList.querySelectorAll('.result-time').forEach(timeEl => {
    timeEl.textContent = 'NT';
    timeEl.dataset.recorded = 'false';
  });
  //change other non result list attributes
  isRunning = false;
  activeMeters = null;
  startTime = null;
}
...
//autonatically uppdate the list on refresh or first entry(hence the need for label.innerHTML = `<span class="calc-icon"><i class="fa-solid fa-calculator"></i></span>Avg ${formatDistanceNumber(displayValue)}`;)
updateDistanceLabels();
const stopwatchDisplay = document.getElementById('stopwatch-display');
const unitSwitch = document.getElementById('unit-switch');
const resetBtn = document.getElementById('reset-btn');
